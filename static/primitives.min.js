...
primitives.orgdiagram.BaseController.prototype._onDefaultTemplateRender = function (event, data) {//ignore jslint
    ...
    data.element.find("[name=person_id]").val(itemConfig.id);
    data.element.find("[name=alive]").val(itemConfig.alive);
    data.element.find("[name=years]").text(itemConfig.years);
};
...
primitives.orgdiagram.Template.prototype._getDefaultItemTemplate = function () {
    ...
    plus = jQuery('<div onclick="add_person_js(this)" name="plus">+</div>')
		.css({
			left: "98px",
			width: "18px",
			height: "17px"
		}).addClass("bp-item bp-plus");
	itemTemplate.append(plus);

    remove = jQuery('<div onclick="remove_person_js(this)" name="remove">x</div>')
		.css({
			left: "98px",
			width: "18px",
			height: "17px"
		}).addClass("bp-item bp-remove");
	itemTemplate.append(remove);
